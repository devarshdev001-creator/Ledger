...
import { Search, Edit2, Trash2, History, TrendingUp, TrendingDown, User, Building2 } from 'lucide-react';
...
export function HistoryView({ transactions, onEdit, onDelete, language, theme }: HistoryViewProps) {
  const t = getTranslation(language);
  const isDark = theme === 'dark';
  const [searchQuery, setSearchQuery] = useState('');
  const [filter, setFilter] = useState<'all' | 'worker' | 'supplier'>('all');
  ...
  const filteredTransactions = searchQuery.trim()
    ? transactions.filter(t => t.name.toLowerCase().includes(searchQuery.toLowerCase()))
    : transactions;
  ...
  return (
    ...
    >
          ...
          >
            <div className="flex items-start justify-between mb-2">
              <div>
                ...
                <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                  {searchSummary.count} transaction(s) found
                </p>
              </div>
            ...
            <div className="flex gap-2 mt-3">
              <div className={`flex-1 rounded-lg p-2 border ${
                ...
                <div className="text-sm text-emerald-600">
                  ₹{searchSummary.totalIn.toLocaleString()}
                </div>
              </div>
              <div className={`flex-1 rounded-lg p-2 border ${
                isDark 
                ...
                <div className="text-sm text-rose-600">
                  ₹{searchSummary.totalOut.toLocaleString()}
                </div>
              </div>
            </div>
          </motion.div>
      ...
      <h3 className={`text-sm mb-4 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
        {searchQuery.trim() ? 'Search Results' : 'All Transactions'}
      </h3>
      ...
      {filteredTransactions.length === 0 && !searchQuery.trim() && (
        ...
        >
          <div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4 ${
            ...
            <History className={`w-10 h-10 ${isDark ? 'text-gray-600' : 'text-gray-400'}`} />
          </div>
